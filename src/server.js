...
app.use((req,res,next)=>{
  const frontendAsset=/\.(?:html|js|css)$/i.test(req.path)||['/','/tester','/preview','/scanner','/api','/api/docs'].includes(req.path);
...
app.get(['/api/health','/api/v1/health'],(_req,res)=>res.json({ok:true,service:'NekoRoute',version:'0.4.1',nodes:pool.nodes.size}));
...
app.get('/api/v1',(_req,res)=>res.json({
  service:'NekoRoute',version:'0.4.1',docs:'/api/docs',openapi:'/api/openapi.json',endpoints:{
...
app.get('/api/openapi.json',(_req,res)=>res.json({openapi:'3.1.0',info:{title:'NekoRoute Public API',version:'0.4.1',description:'Regional availability diagnostics, defensive scanning, proxy-pool metadata and safe preview sessions.'},paths:{
