...
const config = {
  ...
  previewSessionTtlMs:Math.max(60000,int('PREVIEW_SESSION_TTL_MS',900000)), previewMaxHtmlBytes:Math.max(65536,int('PREVIEW_MAX_HTML_BYTES',2097152)),
...
const configHandler=(_req,res)=>res.json({
  exposeNodeAddresses:config.exposeAddresses,matrixMaxNodes:config.matrixMaxNodes,previewSessionTtlMs:config.previewSessionTtlMs,publicTools:true,previewAllowsAllPublicDomains:true,
  ...
  storeScanHistory:config.storeScanHistory,apiVersion:'v1',
...
const proxiesHandler=(req,res)=>{const filters={country:req.query.country?normalizeCountry(req.query.country):undefined,region:req.query.region,protocol:req.query.protocol?.toLowerCase(),status:req.query.status?.toLowerCase()};res.json(pool.list(filters).slice(0,Math.max(1,Math.min(500,Number(req.query.limit||100)))).map(safeNode));};
...
app.get(['/api/health','/api/v1/health'],(_req,res)=>res.json({ok:true,service:'NekoRoute',version:'0.4.0',nodes:pool.nodes.size}));
app.get(['/api/stats','/api/v1/stats'],statsHandler);
app.get(['/api/config','/api/v1/config'],configHandler);
app.get(['/api/proxies','/api/v1/proxies','/api/v1/nodes'],proxiesHandler);
...
app.get('/api/v1',(_req,res)=>res.json({
  service:'NekoRoute',version:'0.4.0',docs:'/api/openapi.json',endpoints:{
    ...
    test:'POST /api/v1/test',matrix:'POST /api/v1/test-matrix',scan:'POST /api/v1/scan',previewSession:'POST /api/v1/preview/session',threatIntel:'GET /api/v1/threat-intel'
  }
}));
app.get('/api/openapi.json',(_req,res)=>res.json({openapi:'3.1.0',info:{title:'NekoRoute Public API',version:'0.4.0'},paths:{
  ...
  '/api/v1/test':{post:{summary:'Test one URL through a selected/best route'}},'/api/v1/test-matrix':{post:{summary:'Compare one URL across multiple routes'}},'/api/v1/scan':{post:{summary:'Defensive website scan through a proxy'}},'/api/v1/preview/session':{post:{summary:'Create a safe preview session'}}
}}));
...
function registerResource(id,session,url,kind='resource'){
  const token=createHash('sha256').update(`${kind}\0${url}`).digest('hex').slice(0,28);
  if(session.resources.size>2000){const first=session.resources.keys().next().value;if(first)session.resources.delete(first);}
  session.resources.set(token,{url,kind});
  return `/api/preview-resource/${encodeURIComponent(id)}/${token}`;
}
function rewriteCss(css,base,id,session){return String(css).replace(/url\(\s*(['"]?)([^'"\)]+)\1\s*\)/gi,(m,_q,v)=>{const raw=v.trim();if(/^(data:|blob:|#)/i.test(raw))return m;const u=absolute(raw,base);return u?`url("${registerResource(id,session,u,'resource')}")`:m;});}
const mediaExt=/\.(?:mp3|m4a|aac|wav|ogg|oga|opus|flac|mp4|m4v|webm|mov|jpg|jpeg|png|gif|webp|avif|svg|pdf)(?:$|[?#])/i;
function rewriteSrcset(value,base,id,session){return String(value||'').split(',').map(part=>{const bits=part.trim().split(/\s+/);const u=absolute(bits[0],base);if(u)bits[0]=registerResource(id,session,u,'resource');return bits.join(' ');}).join(', ');}
function rewriteHtml(html,base,id,session){
  ...
  $('a[href]').each((_i,e)=>{const raw=$(e).attr('href');if(!raw||raw.startsWith('#'))return;const u=absolute(raw,base);if(!u){$(e).attr('href','#').attr('data-neko-blocked','true');return;}const explicit=$(e).is('[download]')||mediaExt.test(u);if(explicit){$(e).attr('href',registerResource(id,session,u,'download')+'?download=1').attr('rel','nofollow noopener');}else{$(e).attr('href','#').attr('data-neko-url',u);}});
  for(const[sel,attr,kind]of[['img','src','resource'],['source','src','media'],['video','src','media'],['video','poster','resource'],['audio','src','media'],['link[rel="stylesheet"]','href','style'],['link[rel="icon"]','href','resource']])$(sel).each((_i,e)=>{const u=absolute($(e).attr(attr),base);if(u)$(e).attr(attr,registerResource(id,session,u,kind)).removeAttr('integrity').removeAttr('crossorigin');});
  ...
  $('body').prepend(`<div style="position:sticky;top:0;z-index:2147483647;padding:8px 12px;background:#07110b;color:#a7f3d0;border-bottom:1px solid #14532d;font:12px system-ui">NekoRoute proxied preview · links, GET forms and page-linked media enabled · remote scripts/cookies/POST forms disabled</div>`);
