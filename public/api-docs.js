...
const endpoints = [
  ...
  { method:'POST', path:'/api/v1/preview/session', title:'Create sandboxed interactive preview session', desc:'Creates an expiring sandboxed preview session pinned to a selected/best route. CSS/JS assets and common GET/HEAD script requests are proxied through that session; the response includes sessionId, frameUrl, node metadata and capabilities.', body:{url:'https://example.com/',country:'RU'}, example:`curl -sS '${base}/api/v1/preview/session' \\\n  -H 'content-type: application/json' \\\n  -d '{"url":"https://example.com/","country":"RU"}'` },
