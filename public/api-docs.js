...
const endpoints = [
  ...
  { method:'GET', path:'/api/v1/nodes?status=online&country=RU&limit=25', title:'Proxy nodes', desc:'Returns public metadata for filtered nodes. Raw addresses stay hidden unless the operator enables EXPOSE_NODE_ADDRESSES.', params:['status — online | degraded | offline | unknown','country — ISO-2 code such as RU, GB, FR','region — e.g. Europe, Asia','protocol — http | https | socks4 | socks5','limit — 1 to 500'], example:`curl -sS '${base}/api/v1/nodes?status=online&country=RU&limit=25'` },
